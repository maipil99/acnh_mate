...
namespace AcnhMateApi.Models; 
    ...
    public class Art: BaseDataObject
    {
        ...
        [BsonElement("museum-desc")]
        public string MuseumDesc { get; set; }

        public override string ToString()
        {
            return $"{nameof(HasFake)}: {HasFake}, {nameof(BuyPrice)}: {BuyPrice}, {nameof(SellPrice)}: {SellPrice}, {nameof(ImageUri)}: {ImageUri}, {nameof(MuseumDesc)}: {MuseumDesc}";
        }
    }